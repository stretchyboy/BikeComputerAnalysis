...
df = pd.read_sql_query("SELECT session_id, lat/1000000.0 as lat, lon/1000000.0 as lon, elev, time, temperature, distance, starttime, overalltime, meters as elevation_gain, weight as bike_weight FROM tracks JOIN sessions ON (tracks.session_id = sessions._id) JOIN bikes ON (used_bike=bikes._id) WHERE cat IN (1,2,3)", con)
...
#df['elapsed_mins'] = ((df['time'] - df['starttime']) / (1000*60) ).astype(int)
df['elapsed_hrs'] = ((df['time'] - df['starttime']) / (1000*60*60) ).astype(int)
...
groups = groups[(groups['grad_mean'] < 20)] 
...
groups["cat"] = pd.cut(groups['score'], [0, 8000, 16000, 32000, 64000, 80000, 1000000], right=True, labels=["-","4", "3", "2", "1", "HC"])

print (groups)

con.close()

means = groups.groupby(['elapsed_hrs','grad_mean']).agg(
        kph = ("kph","mean"),
        #kph_min = ("kph","min"),
        #kph_max = ("kph","max"),
)
